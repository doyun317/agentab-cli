...
- `doctor`는 운영 경로 점검을 위해 `logsDir`, `daemonLogPath`, `pinchtabLogPath`, `artifactsDir`를 함께 노출한다.
...
- 관리형 artifact 응답은 `managed`, `relativePath`, `createdAt`를 포함한다.
...
- 아니면 `${AGENTAB_HOME}`별로 사용 가능한 loopback 포트를 골라 로컬 PinchTab runtime을 분리한다
