...
- GitHub-facing 예제는 README로 정리됐고, 이제 남은 문서 작업은 내부 운영 문서끼리 겹치는 내용을 줄여 유지보수 비용을 낮추는 쪽이 우선이기 때문
...
### 2026-03-18 05:49 UTC

변경:

- GitHub에서 바로 보이는 공식 예제 명령 셋을 `README.md`에 추가했다.
- `doctor -> session start -> tab open -> text -> find/click -> snapshot/screenshot -> cleanup` 흐름을 README 기준으로 정리했다.
- `docs/cli.md`의 중복 예제는 제거하고, README를 공식 예제 위치로 연결했다.

이유:

- 실제 GitHub 공개면에서는 README가 가장 먼저 보이는 진입점이고, 사용자에게 보여줄 대표 명령 흐름도 한 곳에만 두는 편이 더 깔끔하기 때문

영향:

- 공개 예제의 기준 문서가 README로 고정되었다.
- 로컬 docs는 세부 설명과 운영 문서 역할에 더 집중할 수 있게 됐다.
