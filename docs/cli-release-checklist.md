...
- [x] 잘못된 session / tab 지정 시 오류가 일관적이다.
- [x] daemon 재시작 후에도 상태가 비정상적으로 꼬이지 않는다.

### 7.4 액션 안정성

- [x] 클릭, 입력, 스크롤 기본 동작이 실제 페이지에서 재현된다.
  메모: `scripts/smoke-modes.sh` 기준으로 `click`, `type`, `fill`, `press`, `scroll`이 `headless`와 `headed` 모두에서 통과했다.
...
- [x] 필요한 경우 headed / headless 각각 확인
...
- [x] 변경된 공개 계약 정리
- [~] 알려진 제한 사항 정리
  메모: display가 없는 환경에서 `headed` smoke를 돌릴 때는 `Xvfb` 같은 가상 디스플레이가 필요하다.
...
- [x] 남은 이슈 기록
  - 공개 `v0.1.4` asset 기준 `doctor`, `headless`, `headed`, 확장 액션 smoke 재검증까지 완료했다.
